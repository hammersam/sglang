 ...
 */
#ifndef SGL_KERNEL_TOPK_FI_CUH_
#define SGL_KERNEL_TOPK_FI_CUH_

// Adapted from flashinfer PR #2215 (https://github.com/flashinfer-ai/flashinfer/pull/2215)
// Modifications:
//   1. Include paths changed from relative to flashinfer package includes
//   2. Added row_starts parameter for ragged input score layout (sglang-specific)
...
__global__ void __launch_bounds__(BLOCK_THREADS) RadixTopKKernel_Unified(
    ...
    const IdType* row_starts,    // sglang: ragged input offset per row, nullptr if no offset
...
#define shared_output_counter shared_scalars[4]
  ...
  for (uint32_t iter = 0; iter < total_iterations; iter++) {
    ...
    if constexpr (MODE == RadixTopKMode::Basic) {
      length = (lengths != nullptr) ? lengths[row_idx] : stride;  // Per-row or fixed length
    ...
    if constexpr (MODE == RadixTopKMode::Basic) {
      if (k >= length) {
        // k >= length: return all valid indices, fill rest with -1
        const uint32_t chunk_start = cta_in_group * chunk_size;
        const uint32_t chunk_end = min(chunk_start + chunk_size, k);
        const uint32_t row_start = (row_starts != nullptr) ? row_starts[row_idx] : 0;
        for (uint32_t i = tx; i < chunk_end - chunk_start; i += BLOCK_THREADS) {
          const uint32_t idx = chunk_start + i;
          if (idx < length) {
            row_output[idx] = static_cast<IdType>(idx);
            if (output_values != nullptr)
              output_values[row_idx * top_k_val + idx] =
                  input[row_idx * stride + row_start + idx];
          } else if (idx < k) {
            row_output[idx] = static_cast<IdType>(-1);
            if (output_values != nullptr)
              output_values[row_idx * top_k_val + idx] = DType(0);
          }
        }
        continue;
      }
    ...
    LoadToSharedOrdered<BLOCK_THREADS, VEC_SIZE, DType, Traits>(
        input + row_idx * stride + row_start, shared_ordered, chunk_start, actual_chunk_size, tx);
    ...
    if constexpr (MODE == RadixTopKMode::Basic) {
      DType* row_output_values = (output_values != nullptr) ? output_values + row_idx * top_k_val : nullptr;
      RadixCollectIndices<BLOCK_THREADS, SINGLE_CTA, OrderedType>(
          ...
          [&](uint32_t original_idx, OrderedType ordered_val, int pos) {
            ...
            if (row_output_values != nullptr) row_output_values[pos] = Traits::FromOrdered(ordered_val);
          });
    } else if constexpr (MODE == RadixTopKMode::PageTableTransform) {
      uint32_t batch_idx = (row_to_batch != nullptr) ? row_to_batch[row_idx] : row_idx;
      const IdType* src_page_entry = aux_data + batch_idx * aux_stride;
  ...
  DISPATCH_ALIGNED_VEC_SIZE(vec_size, VEC_SIZE, {
    if (single_cta) {
      ...
      void* args[] = {&input,         &output_page_table, &output_values,     &src_page_table,
                      &lengths,       &row_to_batch,      &row_starts,        &src_stride,
                      &top_k_val,     &max_len,           &num_rows,          &row_states_buffer,
  ...
  DISPATCH_ALIGNED_VEC_SIZE(vec_size, VEC_SIZE, {
    if (single_cta) {
      ...
      void* args[] = {&input,         &output_indices, &output_values,     &offsets,
                      &lengths,       &row_to_batch,   &row_starts,        &aux_stride,
                      &top_k_val,     &max_len,        &num_rows,          &row_states_buffer,
                      &chunk_size,    &ctas_per_group};
      FLASHINFER_CUDA_CALL(cudaLaunchKernel((void*)kernel, nblks, nthrs, args, smem_size, stream));
    } else {
...
cudaError_t RadixTopKMultiCTA(DType* input, IdType* output_indices, DType* output_values,
                              IdType* top_k_arr, IdType* per_row_lengths,
                              const IdType* row_starts,
                              uint32_t batch_size, uint32_t top_k_val,
                              uint32_t vocab_size, RadixRowState* row_states_buffer,
                              cudaStream_t stream = 0) {
  ...
  // Unified kernel parameters
  IdType* lengths = per_row_lengths;     // Per-row lengths (nullptr = use vocab_size for all)
...
__global__ void __launch_bounds__(FILTERED_TOPK_BLOCK_THREADS)
    FilteredTopKUnifiedKernel(const DType* __restrict__ input, IdType* __restrict__ output,
                              ...
                              const IdType* __restrict__ row_starts,    // sglang: ragged input offset
  ...
  const IdType row_start = (row_starts != nullptr) ? row_starts[bid] : 0;
  const DType* score = input + bid * max_len + row_start;
...
cudaError_t FilteredTopKPageTableTransform(DType* input, IdType* output_page_table,
  ...
  void* args[] = {&input,        &output_page_table, &aux_output, &src_page_table, &src_stride,
                  &row_to_batch, &lengths,           &row_starts,
...
cudaError_t TopKPageTableTransformDispatch(DType* input, IdType* output_page_table,
  ...
  if (ShouldUseFilteredTopK<DType>(num_rows, top_k_val, max_len)) {
    return FilteredTopKPageTableTransform<DType, IdType>(input, output_page_table, src_page_table,
                                                         ...
                                                         row_starts, num_rows, top_k_val, max_len, stream);
  }
  return RadixTopKPageTableTransformMultiCTA<DType, IdType>(
      ...
      row_starts, num_rows, top_k_val, max_len, row_states_buffer, stream);
}
...
cudaError_t TopKDispatch(DType* input, IdType* output_indices, DType* output_values,
  ...
  if (ShouldUseFilteredTopK<DType>(num_rows, top_k_val, max_len)) {
    return FilteredTopK<DType, IdType>(input, output_indices, output_values, lengths, row_starts,
                                       num_rows, top_k_val, max_len, stream);
  }
  return RadixTopKMultiCTA<DType, IdType>(input, output_indices, output_values, nullptr, lengths,
                                          row_starts, num_rows, top_k_val, max_len,
                                          row_states_buffer, stream);
}
